...
All 120 planned specifications converged, so the internal meta-analysis covers the complete design space. Table~\ref{tab:dist} reports the distribution of the resulting scarcity coefficients, and Table~\ref{tab:meta} reports the meta-regression coefficients. The average estimate is -0.292, the median is -0.144, the interquartile range runs from -0.364 to -0.035, 85.8\% of the estimates are negative, and 29.2\% are statistically significant at the 5\% level. These moments already suggest that the sign of the relationship is usually stable even when the magnitude is not.
\begin{figure}[H]
\centering
...
\input{../tables/meta_regression.tex}
...
Relative to the two-way fixed-effects linear baseline, the quadratic specification lowers the coefficient by -0.152 (p=0.003), the income interaction lowers it by -0.772 (p=0.000), and the threshold model lowers it by -0.224 (p=0.000). These are economically meaningful changes: each one makes the scarcity effect substantially more negative.
Estimator choice also matters. Dropping year fixed effects makes the coefficient -0.120 more negative on average (p=0.002), while random-effects and between estimators shift the coefficient upward by 0.189 (p=0.012) and 0.222 (p=0.007), respectively. This pattern is consistent with the idea that the within-country signal is more negative than the between-country correlation.
The quantile results reinforce the paper's tail-risk interpretation. The Q25 and Q50 coefficients are -0.157 and -0.110 below the baseline meta-regression constant, while the Q75 shift is -0.072 (p=0.211). Lower-tail growth episodes therefore appear especially sensitive to water scarcity.
\begin{figure}[H]
\centering
